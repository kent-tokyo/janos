...
- CSAクライアントでfloodgate接続中（アカウントは `.env` の `FLLODGATE_ACCOUNT` で設定）
...
crates/
  ...
  usi/          — USIサーバー → バイナリ: sekirei
  csa/          — floodgate CSAクライアント → バイナリ: sekirei-csa
  match-runner/ — USI対USI棋力テスト管理 → バイナリ: sekirei-match
